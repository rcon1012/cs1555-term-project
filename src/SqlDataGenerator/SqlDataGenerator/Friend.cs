...
namespace SqlDataGenerator {
    ...
    public class Friend {
        public Friend(Profile friend1, Profile friend2) {
            ...
            this.status = Util.rand.Next((int)FriendStatus.Pending, (int)FriendStatus.Count);
            if(status == (int)FriendStatus.Pending) {
                this.established = Util.RandomDateTime(Util.Max(friend1.dob, friend2.dob), friend1.last_on); // Can occur from birth to when the requester was last online
            } else {
